...
const renderMessageContent = (content, onButtonClick) => {
  ...
  if (content.startsWith("[") && content.endsWith("]")) {
    return (
      <span
        style={{ color: "#64748b", fontStyle: "italic", fontWeight: "500" }}
      >
        {content}
      </span>
    );
    ...
    try {
      ...
      if (parsed.type === "interactive" && parsed.interactive) {
        ...
        if (parsed.interactive.type === "cta_url") {
          const param = parsed.interactive.action?.parameters || {};
          return (
            <div>
              <div style={{ whiteSpace: "pre-wrap" }}>{text}</div>
              {param.url && (
                <div style={{ marginTop: "10px" }}>
                  <a href={param.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "block" }}>
                    <div 
            ...
            >
              {parsed.quick_replies.map((qr, idx) => (
                <button
                  ...
                  onClick={() =>
                    onButtonClick &&
                    onButtonClick(qr.title, qr.payload, "quick_reply")
                  }
                  style={{
                    fontSize: "12px",
                    background: "#f8fafc",
                ...
                {element.buttons.map((btn, idx) => {
                  ...
                  return isUrl ? (
                    ...
                    <button
                      ...
                      onClick={() =>
                        onButtonClick &&
                        onButtonClick(btn.title, btn.payload, "postback")
                      }
                      style={{
                        width: "100%",
                        fontSize: "13px",
...
export default function Inbox() {
          ...
          entry: [
            {
              ...
              changes: [
                {
                  field:
                    type === "postback" ? "messaging_postbacks" : "messages",
                  value:
                    type === "postback"
                      ? {
                          from: {
  ...
  useEffect(() => {
    if (!clientId) return;
    contactsAPI
      .getAll({ client_id: clientId, limit: 50 })
    ...
    if (isServerless) {
      ...
      const interval = setInterval(async () => {
        try {
          const rContacts = await contactsAPI.getAll({
            client_id: clientId,
            limit: 50,
          });
          ...
          if (selected?.id) {
            const rMsgs = await messagesAPI.getAll({
              client_id: clientId,
              contact_id: selected.id,
            });
            setMessages((prev) => {
      ...
      socketRef.current.on("new_message", (msg) => {
        ...
        toast(`New message from ${msg.contact_name || "contact"}`, {
          icon: "💬",
        });
  ...
  const sendMessage = async () => {
    ...
    if (isSimulateMode) {
      ...
      setMessages((prev) => [
        ...prev,
        { direction: "inbound", content: text, created_at: new Date() },
      ]);
      ...
      const toastId = toast.loading(
        isWhatsApp
          ? "Simulating WhatsApp customer message..."
          : "Simulating customer text message...",
      );
      try {
        if (isWhatsApp) {
          await metaAPI.simulateWhatsAppWebhook(webhookPayload, clientId);
      ...
      setMessages((prev) => [
        ...prev,
        { direction: "outbound", content: text, created_at: new Date() },
      ]);
      try {
  ...
  const platColor = (p) =>
    ({ whatsapp: "#25D366", instagram: "#E1306C", facebook: "#1877F2" })[p] ||
    "#888";
              ...
              >
                <span
                  style={{
                    ...
                    padding: "1px 6px",
                    borderRadius: 20,
                    background: platColor(c.platform) + "20",
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#8892a4" }}>
                {selected.platform} · {selected.lead_status}
              </div>
            </div>
            {/* Open WhatsApp */}
                        ...
                        : {
                            ...
                            background:
                              m.direction === "outbound"
                                ...
                                : "#fff",
                            color:
                              m.direction === "outbound" ? "#fff" : "#0a0f1e",
                            border:
                              m.direction === "inbound"
                                ? "0.5px solid #e5e7eb"
                                : "none",
          ...
          >
            <button
              ...
              title={
                isSimulateMode
                  ? "Currently in Customer Simulation mode (messages will be sent from customer to bot)"
                  : "Currently in Admin Reply mode (messages will be sent from bot to customer)"
              }
              style={{
                padding: "9px 12px",
            ...
            <input
              ...
              placeholder={
                isSimulateMode
                  ? "Type simulated message from customer..."
                  : `Reply via ${selected.platform}...`
              }
              style={{
                flex: 1,
                padding: "9px 12px",
